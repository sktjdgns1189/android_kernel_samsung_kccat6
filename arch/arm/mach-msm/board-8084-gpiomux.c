...
#if !defined(CONFIG_SEC_TRLTE_DCM) && !defined(CONFIG_SEC_TRLTE_KDI)
	/* APQ_CTI_PAIR2_OUT */
	{
		.gpio = 124,
		.settings = {
...
static struct msm_gpiomux_config gpio_nc_configs[] __initdata = {
	...
	GPIOMUX_SET_NC(117),	/* CHINA: AF_OIS_TDMB_SCL */
...
void __init apq8084_init_gpiomux(void)
{
    ...
    if (system_rev > 7) {
		...
		if(system_rev >= 7) {
			apq8084_senn3ab_configs[0].gpio = GPIO_FELICA_VEN_REV07;
		} 
