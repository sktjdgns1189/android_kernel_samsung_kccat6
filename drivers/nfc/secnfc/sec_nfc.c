...
// JY Kim - Security
    char *cmdpos;
    static char cmdline[SEC_NFC_PROCESS_NAME_SIZE];
    int leng = SEC_NFC_PROCESS_NAME_SIZE;
	dev_dbg(info->dev, "%s: info : %p" , __func__, info);

    cmdpos = (char *)(current->mm->arg_start);
    ret = copy_from_user(cmdline, cmdpos, leng);

    dev_dbg(info->dev, "%s: Process name = %s", __func__, cmdline);

    // Check process
    if(strcmp(cmdline, SEC_NFC_PROCESS_NAME))
    {
        //pr_info("%s: Not allowed process. Device node will be removed!\n", __func__);
        //return -EBUSY;
    }
