...
#define BIN_MD_VER_LOCATION	0x04
#define BIN_CRC_LOCATION	0x0c
#endif
...
#ifdef CRC_CHECK_INTERNAL
	info->fw_crc = ((0xFF & info->fw->data[BIN_CRC_LOCATION + 1]) << 8)\
				| info->fw->data[BIN_CRC_LOCATION];
#endif
...
static int tkey_flash_fw(struct cypress_touchkey_info *info, u8 fw_path, bool force)
{
	...
	/* CY device bit 6 (0x40) = MBR3155 */
	/* Do FW update after 0x05 module */
	if (force == true || \
		(info->device_ver == 0x40 && info->module_ver >= 0x05))
