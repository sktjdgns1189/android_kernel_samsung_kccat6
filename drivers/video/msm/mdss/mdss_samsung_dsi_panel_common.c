...
static struct dsi_cmd hmt_dual_scan_enable;
...
static struct dsi_cmd aid_cmds_list_reverse_hmt_dual;
static struct dsi_cmd aid_cmds_list_reverse_hmt_single;
static struct dsi_cmd aid_cmds_list_reverse_hmt_dual_80;
static struct dsi_cmd aid_cmds_list_reverse_hmt_single_80;
static struct candella_lux_map candela_map_table_reverse_hmt;
extern int skip_te_enable;
static int is_first = 1;

/*
static struct dsi_cmd mps_elvss_cmds_list_hmt;
...
static int mdss_dsi_set_col_page_addr(struct mdss_panel_data *pdata)
{
	...
	if (mdss_dsi_broadcast_mode_enabled())
		update = 1;
	else if (!mdss_rect_cmp(c_roi, p_roi))	/* roi had changed */
		update = 1;
...
*/
static struct cmd_map* get_aid_map_table_hmt(void) {
	struct cmd_map *aid_map_table = NULL;

	aid_map_table = &aid_map_table_reverse_hmt;

	return aid_map_table;
}

static struct dsi_cmd* get_aid_cmds_list_hmt(void) {
	struct dsi_cmd *aid_cmds_list = NULL;

	if (msd.hmt_stat.hmt_aor == 80) {
		if (msd.hmt_stat.hmt_dual == HMT_DUAL_SCAN) {
			aid_cmds_list = &aid_cmds_list_reverse_hmt_dual_80;
		} else {
			aid_cmds_list = &aid_cmds_list_reverse_hmt_single_80;
		}
	} else {
		if (msd.hmt_stat.hmt_dual == HMT_DUAL_SCAN) {
			aid_cmds_list = &aid_cmds_list_reverse_hmt_dual;
		} else {
			aid_cmds_list = &aid_cmds_list_reverse_hmt_single;
		}
	}

	return aid_cmds_list;
}
#if 0
static struct dsi_cmd get_mps_elvss_control_set_hmt(void)
{
	struct dsi_cmd mps_elvss_control = {0,};

	if (msd.dstat.acl_on||msd.dstat.siop_status)
		mps_elvss_cmds_list_hmt.cmd_desc[0].payload[1] = 0x8C;
	else
		mps_elvss_cmds_list_hmt.cmd_desc[0].payload[1] = 0x9C;

	mps_elvss_control = mps_elvss_cmds_list_hmt;

	return mps_elvss_control;
}
...
static struct dsi_cmd get_aid_aor_control_set_hmt(int cd_idx)
{
	...
	struct cmd_map *aid_map_table = get_aid_map_table_hmt();
	struct dsi_cmd *aid_cmds_list = get_aid_cmds_list_hmt();
...
static struct smartdim_conf_hmt* get_smartdim_conf_hmt(void) {
	...
	if (msd.hmt_stat.hmt_aor == 80) {
		if (msd.hmt_stat.hmt_dual == HMT_DUAL_SCAN) {
			conf_hmt = msd.sdimconf_reverse_hmt_dual_80;
		} else {
			conf_hmt = msd.sdimconf_reverse_hmt_single_80;
		}
	} else {
		if (msd.hmt_stat.hmt_dual == HMT_DUAL_SCAN) {
			conf_hmt = msd.sdimconf_reverse_hmt_dual;
		} else {
...
#if 0
static struct dsi_cmd get_gamma_control_set_hmt(int candella)
{
	struct dsi_cmd gamma_control = {0,};
	/* Just a safety check to ensure smart dimming data is initialised well */

	struct smartdim_conf_hmt *conf_hmt = get_smartdim_conf_hmt();

	BUG_ON(conf_hmt->generate_gamma == NULL);
	conf_hmt->generate_gamma(candella, &hmt_bright_cmds_list.cmd_desc[0].payload[1], msd.hmt_stat.hmt_dual, msd.hmt_stat.hmt_reverse);

	gamma_control.cmd_desc = &(hmt_bright_cmds_list.cmd_desc[0]);
	gamma_control.num_of_cmds = hmt_bright_cmds_list.num_of_cmds;
	return gamma_control;
}
#endif
...
static void make_brightcontrol_set_hmt(int bl_level)
{
	int cmd_idx = 0, cd_idx = 0, cd_level =0;
	struct cmd_map *aid_map_table;
	struct dsi_cmd *aid_cmds_list;
	struct smartdim_conf_hmt *conf_hmt;

	aid_map_table = get_aid_map_table_hmt();
	aid_cmds_list = get_aid_cmds_list_hmt();
	...
	BUG_ON(conf_hmt->generate_gamma == NULL);
	conf_hmt->generate_gamma(cd_level, &hmt_bright_cmds_list.cmd_desc[1].payload[1], msd.hmt_stat.hmt_dual);
	...
	cmd_idx = aid_map_table->cmd_idx[cd_idx];
	hmt_bright_cmds_list.cmd_desc[2] = aid_cmds_list->cmd_desc[cmd_idx];
...
static int make_brightcontrol_set_hmt(int bl_level)
{
	...
	/*gamma CA */
	gamma_control = get_gamma_control_set_hmt(cd_level);
	cmd_count = update_bright_packet(cmd_count, &gamma_control);
	...
	/* MPS/ELVSS B6 */
	mps_elvss_control = get_mps_elvss_control_set_hmt();
	cmd_count = update_bright_packet(cmd_count, &mps_elvss_control);
		...
		unsigned char lock)
{
	...
	switch (cmd) {
		...
		case PANEL_HMT_BRIGHT:
			...
			make_brightcontrol_set_hmt(msd.hmt_stat.hmt_bl_level);
			cmd_desc = hmt_bright_cmds_list.cmd_desc;
			cmd_size = hmt_bright_cmds_list.num_of_cmds;
			break;
		case PANEL_DUAL_SCAN_FULL_ENABLE:
			cmd_desc = hmt_dual_scan_enable.cmd_desc;
			cmd_size = hmt_dual_scan_enable.num_of_cmds;
			break;
		case PANEL_SINGLE_SCAN_ENABLE:
...
#if defined(CONFIG_LCD_HMT)
static void mdss_dsi_panel_make_sdimconf(struct mdss_panel_data *pdata, struct smartdim_conf_hmt *pSdimconf, struct candella_lux_map *pCandela_map_table, int dual) {
	int size;
	...
	pr_info("[HMT] smart dimming done! [%d]\n", dual);
}
...
static int mdss_dsi_panel_dimming_init_HMT(struct mdss_panel_data *pdata)
{
	...
	msd.hmt_stat.hmt_bright_mode = 0;
	msd.hmt_stat.hmt_aor = 70;

	switch (msd.panel) {
		case PANEL_WQHD_OCTA_S6E3HA2X01_CMD:
			pr_info("here!!\n");
			msd.sdimconf_reverse_hmt_dual = smart_S6E3_get_conf_hmt(HMT_DUAL_SCAN);
			msd.sdimconf_reverse_hmt_single = smart_S6E3_get_conf_hmt(HMT_SINGLE_SCAN);
			break;
	}

	mdss_dsi_panel_make_sdimconf(pdata, msd.sdimconf_reverse_hmt_dual, &candela_map_table_reverse_hmt, HMT_DUAL_SCAN);
	mdss_dsi_panel_make_sdimconf(pdata, msd.sdimconf_reverse_hmt_single, &candela_map_table_reverse_hmt, HMT_SINGLE_SCAN);
...
static void mdss_dsi_panel_make_sdimconf_80(struct mdss_panel_data *pdata, struct smartdim_conf_hmt *pSdimconf, struct candella_lux_map *pCandela_map_table, int dual) {
	int size;
	...
	/* Just a safety check to ensure smart dimming data is initialised well */
	BUG_ON(pSdimconf->init_80 == NULL);
	pSdimconf->init_80(dual);

	pr_info("[HMT] smart dimming done! [%d]\n", dual);
}

static int mdss_dsi_panel_dimming_init_HMT_80(struct mdss_panel_data *pdata)
{
	pr_info("[HMT] %s : ++\n",__func__);


	msd.sdimconf_reverse_hmt_dual_80 = smart_S6E3_get_conf_hmt_80(HMT_DUAL_SCAN);
	msd.sdimconf_reverse_hmt_single_80 = smart_S6E3_get_conf_hmt_80(HMT_SINGLE_SCAN);

	mdss_dsi_panel_make_sdimconf_80(pdata, msd.sdimconf_reverse_hmt_dual_80, &candela_map_table_reverse_hmt, HMT_DUAL_SCAN);
	mdss_dsi_panel_make_sdimconf_80(pdata, msd.sdimconf_reverse_hmt_single_80, &candela_map_table_reverse_hmt, HMT_SINGLE_SCAN);
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	if (!msd.dstat.is_hmt_smart_dim_loaded) {
		...
		mdss_dsi_panel_dimming_init_HMT_80(pdata);
	}
#endif
	...
	if ((msd.panel == PANEL_WQHD_OCTA_S6E3HA2X01_CMD || msd.panel == PANEL_WQXGA_OCTA_S6E3HA2X01_CMD) && (msd.id2 == 0x20))
		mipi_samsung_disp_send_cmd(PANEL_HSYNC_ON, true);
		...
		struct mdss_panel_info *pinfo)
{
	...
	len /= sizeof(u32);
	if (!data || (len != 6)) {
		pr_info("%s: Panel roi alignment not found\n", __func__);
	} else {
...
mdss_samsung_parse_panel_table(np, &smart_vint_map_table,
	...
	mdss_samsung_parse_panel_cmd(np, &hmt_dual_scan_enable,
	...
	mdss_samsung_parse_panel_cmd(np, &aid_cmds_list_reverse_hmt_dual,
				"samsung,panel-aid-cmds-list-reverse-hmt-dual");
	mdss_samsung_parse_panel_cmd(np, &aid_cmds_list_reverse_hmt_single,
				"samsung,panel-aid-cmds-list-reverse-hmt-single");
	mdss_samsung_parse_panel_cmd(np, &aid_cmds_list_reverse_hmt_dual_80,
				"samsung,panel-aid-cmds-list-reverse-hmt-dual-80");
	mdss_samsung_parse_panel_cmd(np, &aid_cmds_list_reverse_hmt_single_80,
				"samsung,panel-aid-cmds-list-reverse-hmt-single-80");
...
/*
	mdss_samsung_parse_panel_cmd(np, &mps_elvss_cmds_list_hmt,
				"samsung,panel-mps-elvss-list-hmt");
...
#if defined(CONFIG_LCD_HMT)
	if (msd.dstat.is_hmt_smart_dim_loaded) {
		msd.sdimconf_reverse_hmt_dual->print_aid_log();
		msd.sdimconf_reverse_hmt_single->print_aid_log();
		msd.sdimconf_reverse_hmt_dual_80->print_aid_log();
		msd.sdimconf_reverse_hmt_single_80->print_aid_log();
	} else
...
/*
 * level 80, 95, 115, 130 -> update AID + GAMMA (forward AID)
 * level 35, 50, 65, 80, 95, 105 -> update AID + GAMMA (reverse AID wqhd)
 */
...
int hmt_dual_scan_update(void)
{
	if (msd.hmt_stat.hmt_dual == HMT_SINGLE_SCAN) {
		...
		msleep(20);
		mipi_samsung_disp_send_cmd(PANEL_SINGLE_SCAN_ENABLE, true);
		pr_info("Single Scan Enable -- \n");
...
static ssize_t mipi_samsung_hmt_bright_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int rc;

	rc = snprintf((char *)buf, sizeof(buf), "%d\n", msd.hmt_stat.hmt_bright_mode);
	pr_info("[HMT] bright mode : %d\n", *buf);

	return rc;
}

static ssize_t mipi_samsung_hmt_bright_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	int input, level;
	int bl_level, cd, bkl_size;

	struct candella_lux_map *map_table = get_candella_lux_map_hmt();

	sscanf(buf, "%d %d" , &input, &level);
	pr_info("[HMT] %s: input (%d) level(%d)\n", __func__, input, level);
	...
	// find candella : 1 ~ map_table->lux_tab_size
	if (level > map_table->lux_tab_size) {
		level = map_table->lux_tab_size;
	} else if (level <= 0) {
		level = 1;
	}

	cd = map_table->lux_tab[level-1];

	// set candella map table for low persistent mode
	if (input) {
		map_table = &candela_map_table;
	}

	// find bl_level
	bkl_size = sizeof(map_table->bkl) / sizeof(int);
	for (bl_level = 0; bl_level < bkl_size-1; bl_level++) {
		if (cd <= map_table->lux_tab[map_table->bkl[bl_level]]) {
			break;
		}
	}

	msd.hmt_stat.hmt_bright_mode = input;
	msd.hmt_stat.hmt_bl_level = bl_level;
	hmt_bright_update();
...
static ssize_t mipi_samsung_hmt_aor_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int rc;

	rc = snprintf((char *)buf, sizeof(buf), "%d\n", msd.hmt_stat.hmt_aor);
	pr_info("[HMT] hmt aor : %d\n", *buf);

	return rc;
}

static ssize_t mipi_samsung_hmt_aor_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	int input;
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (msd.hmt_stat.hmt_reverse == input) {
		pr_info("[HMT] hmt reverse already %d!\n", msd.hmt_stat.hmt_reverse);
		return size;
	}
...
static DEVICE_ATTR(hmt_aor, 660,
			mipi_samsung_hmt_aor_show,
			mipi_samsung_hmt_aor_store);
static DEVICE_ATTR(hmt_dual_scan, 660,
			mipi_samsung_hmt_dual_scan_show,
			mipi_samsung_hmt_dual_scan_store);
static DEVICE_ATTR(hmt_reverse, 660,
			mipi_samsung_hmt_reverse_show,
			mipi_samsung_hmt_reverse_store);
static DEVICE_ATTR(hmt_bright_mode, 660,
			mipi_samsung_hmt_bright_mode_show,
			mipi_samsung_hmt_bright_mode_store);
/*
...
static struct attribute *panel_sysfs_attributes[] = {
	...
	&dev_attr_hmt_bright_mode.attr,
	&dev_attr_hmt_aor.attr,
