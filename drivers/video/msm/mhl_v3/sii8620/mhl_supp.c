	...
	if (dev_context->mhl_connection_event) {
		...
		case MHL_MSC_MSG_RCP:
			if ((uint8_t)dev_context->msc_msg_data == 0x7E) {
				MHL_TX_DBG_INFO(": MHL switch event sent : 1\n");
...
static void sii8620_setup_charging(struct mhl_dev_context *dev_context)
{
	...
	if ((version & 0xf0) >= 0x20) {
		...
		MHL_TX_DBG_INFO(": version: 0x%02x, id:%d, reserved:%d\n", version, adopter_id, dev_context->dev_cap_cache.mdc.reserved);
				...
				uint8_t data1)
{
	...
	if (req->status.flags.cancel == true) {
		MHL_TX_DBG_DEBUG("Canceling request with command 0x%02x\n",
				req->command);
		...
		MHL_TX_DBG_DEBUG("mhl: peer XDEVCAP data1:0x%02x\n", data1);
		...
		for (i = 0; i < XDEVCAP_OFFSET(XDEVCAP_LIMIT); ++i) {
			...
			if (xdevcap_changes.xdevcap_cache[i]) {
				MHL_TX_DBG_DEBUG("XDEVCAP[%d] changed from "
					"0x%02x to 0x%02x\n", i,
					old_xdevcap.xdevcap_cache[i],
					dev_context->xdev_cap_cache.
					xdevcap_cache[i]);
		...
		MHL_TX_DBG_DEBUG("mhl: peer XDEVCAP[0x%02x] data1:0x%02x\n",
				req->reg, data1);
		dev_context->xdev_cap_cache.
