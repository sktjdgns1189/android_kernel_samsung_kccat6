...
int sii8620_detection_callback(unsigned long event)
{
	...
	MHL_TX_DBG_INFO(": sii8620_detection_callback : event=%lu*****\n",
			event);

	if (dev_context->muic_state == event)
	{
		MHL_TX_DBG_INFO(": already %s\n", event ? "attached" : "detached");
		return 0;
	}
...
#else
	if (event) {
		MHL_TX_DBG_INFO(": MHL-Connection\n");
		clk_prepare_enable(mhl_clk);
		dev_context->pdata->power(true);
		si_mhl_tx_initialize(dev_context);
		...
		MHL_TX_DBG_INFO(": MHL-Disconnection\n");
		clk_disable_unprepare(mhl_clk);
		disable_irq(dev_context->drv_info->irq);
		dev_context->pdata->charger_mhl_cb(false, -1);
		dev_context->pdata->power(false);
...
#else
	if (event) {
		MHL_TX_DBG_INFO(":detection started\n");
		dev_context->pdata->power(true);
		si_mhl_tx_initialize(dev_context);
		enable_irq(dev_context->drv_info->irq);
	} else {
		MHL_TX_DBG_INFO(":disconnection\n");
		disable_irq(dev_context->drv_info->irq);
		dev_context->pdata->charger_mhl_cb(false, -1);
		dev_context->pdata->power(false);
		if (dev_context->mhl_event_switch.state == 1) {
		...
		char *buf)
{
	...
	MHL_TX_DBG_INFO(": get Device_ID, spi read : ret_val = %04x\n",
			ret_val);
	pdata->power(0);
