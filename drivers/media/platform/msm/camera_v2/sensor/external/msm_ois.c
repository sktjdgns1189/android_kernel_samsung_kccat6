...
void msm_ois_i2c_byte_read(struct msm_ois_ctrl_t *a_ctrl, uint32_t addr, uint16_t *data);
void msm_ois_i2c_byte_write(struct msm_ois_ctrl_t *a_ctrl, uint32_t addr, uint16_t data);
...
static int32_t msm_ois_read_phone_ver(struct msm_ois_ctrl_t *a_ctrl)
{
    ...
    char    ois_hwinfo[OIS_HW_VER_SIZE+1] = "63A";
    int ret = 0, i;
...
static int32_t msm_ois_fw_update(struct msm_ois_ctrl_t *a_ctrl)
{
    ...
    for(block = 4; block < 8; block ++)
    {
        for(i = 0; i < 4; i ++)
        {
            ...
            if(ret < 0)
                pr_err("i2c byte boot code write failed\n");
    ...
    if(val == 0x0100) // check if there is error during boot code update ( 0x0100 : NO_ROM )
    {
        ...
        for(block = 4; block <= 27; block ++)
        {
            for(i = 0; i < 4; i ++)
            {
                ...
                ret = a_ctrl->i2c_client.i2c_func_tbl->i2c_write_seq(
                    &a_ctrl->i2c_client, 0x100, SendData, 256);
                if(ret < 0)
                    pr_err("i2c byte prog code write failed\n");
        ...
        if(val ==0x0000)
        {
            pr_err(" progCode update success \n ");
        }
        else
        {
            pr_err(" progCode update failed, error status val 0x6 0x7 : %04x \n",val);

        }
    }
    else {
        pr_err(" bootCode update failed, error status val 0x6 0x7 : %04x \n",val);
    }
    ...
    void __user *argp)
{
    ...
    case CFG_OIS_READ_MODULE_VER:
        ...
        if (rc < 0)
            pr_err("read module version failed %d\n", rc);

        memcpy(cdata->version, &a_ctrl->module_ver, sizeof(struct msm_ois_ver_t));
        pr_err("(user) version = %s\n", cdata->version);
        break;
    ...
    case CFG_OIS_READ_PHONE_VER:
        ...
        if(isalnum(a_ctrl->module_ver.gyro_sensor))
        {
            ...
            if (rc < 0)
                pr_err("read phone version failed. %d\n", rc);

            memcpy(cdata->version, &a_ctrl->phone_ver, sizeof(struct msm_ois_ver_t));
            pr_err("(user) version = %s\n", cdata->version);
        }
        break;
...
void msm_is_ois_version(void)
{
    ...
    uint16_t version = 0;
    pr_err("msm_is_ois_version : start \n");
...
void msm_is_ois_offset_test(long *raw_data_x, long *raw_data_y, bool is_need_cal)
{
    ...
    uint16_t val;
    uint16_t x = 0, y = 0;
    uint16_t x_sum = 0, y_sum = 0, sum = 0;
    int retries = 0, avg_count = 20;

    pr_err("msm_is_ois_offset_test : start \n");

    if(is_need_cal)
    {
        ret = g_msm_ois_t->i2c_client.i2c_func_tbl->i2c_write(
            &g_msm_ois_t->i2c_client, 0x14, 0x01,
            MSM_CAMERA_I2C_BYTE_DATA);
    ...
    for (i = 0; i < retries; retries--) {
        ...
        g_msm_ois_t->i2c_client.i2c_func_tbl->i2c_read( //Read 0x0248 value
        &g_msm_ois_t->i2c_client, 0x0248, &val,
        MSM_CAMERA_I2C_BYTE_DATA);

        x = val;

        g_msm_ois_t->i2c_client.i2c_func_tbl->i2c_read( //Read 0x0249 value
        &g_msm_ois_t->i2c_client, 0x0249, &val,
        MSM_CAMERA_I2C_BYTE_DATA);
    ...
    *raw_data_x = sum *1000 /175 /10;
    ...
    for (i = 0; i < retries; retries--) {
        ...
        g_msm_ois_t->i2c_client.i2c_func_tbl->i2c_read( //Read 0x024A value
        &g_msm_ois_t->i2c_client, 0x024A, &val,
        MSM_CAMERA_I2C_BYTE_DATA);

        y = val;

        g_msm_ois_t->i2c_client.i2c_func_tbl->i2c_read( //Read 0x024B value
        &g_msm_ois_t->i2c_client, 0x024B, &val,
        MSM_CAMERA_I2C_BYTE_DATA);
...
static ssize_t gyro_selftest_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    pr_err("gyro_selftest_show : start \n");

    if((g_msm_ois_t->i2c_client.client==NULL)&&(g_msm_ois_t->pdev==NULL)) {
        return sprintf(buf, "error, ois module has not been probed");
    }
...
/*Gyro sensor get rawdata (without cal, except that same with selftest) sysfs*/
            ...
            struct device_attribute *attr, char *buf)
{
    ...
    pr_err("gyro_rawdata_test_show : start \n");
...
char ois_fw_full[40] = "NULL NULL NULL\n";
...
char ois_fw_load[25] = "NULL NULL\n";
static ssize_t ois_fw_load_show(struct device *dev,
            struct device_attribute *attr, char *buf)
{
    CDBG("[FW_DBG] OIS_load_fw : %s\n", ois_fw_load);
    return snprintf(buf, sizeof(ois_fw_load), "%s", ois_fw_load);
}

static ssize_t ois_fw_load_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t size)
{
    CDBG("[FW_DBG] buf : %s\n", buf);
    snprintf(ois_fw_load, sizeof(ois_fw_load), "%s\n", buf);
    return size;
}
...
static DEVICE_ATTR(ois_fw_load, S_IRUGO|S_IWUSR|S_IWGRP, ois_fw_load_show, ois_fw_load_store);
...
static int __init msm_ois_init_module(void)
{
    ...
    if (!IS_ERR(camera_class)) { //for sysfs
        ...
        if (device_create_file(cam_ois, &dev_attr_ois_fw_load) < 0) {
            printk("Failed to create device file!(%s)!\n",
                dev_attr_ois_fw_load.attr.name);
            rc = -ENODEV;
        }
