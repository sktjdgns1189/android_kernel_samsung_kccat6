...
static void es705_switch_route_config(long route_index)
{
	...
	if ((route_config_idx == 2) || (route_config_idx == 4) ||
		(route_config_idx == 1) || (route_config_idx == 3)) {
...
#if defined(AUDIENCE_CVQ_DEBUG)
static int es70x_msg_sync(void)
{
	struct es705_priv *es705 = &es705_priv;
	int rc = 0;
	u32 sync_cmd = (ES705_SYNC_CMD << 16) | ES705_SYNC_POLLING;
	...
	memcpy((char *)&sync_ack, msg, 4);
	le32_to_cpus(&sync_ack);
	dev_info(es705->dev, "%s(): sync_ack = 0x%08x\n", __func__, sync_ack);

	return rc;
}

static int es70x_fw_building_string(void)
{
	struct es705_priv *es705 = &es705_priv;
	int idx = 0;
	unsigned int value;
	...
	*verbuf++ = (value & 0x00ff);
	...
	/* Null terminate the string*/
	*verbuf = '\0';
	dev_info(es705->dev, "Audience fw ver %s\n", versionbuffer);
	return idx;
}
#endif
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	...
	u32 cmd_lpsd[] = {0x9017e03c, 0x901800c8,
			...
			0xffffffff};
#if defined(AUDIENCE_CVQ_DEBUG)
	u32 memorycheck_cmd = 0x8024000e;
#endif
	int match = 1;
...
#ifdef SEAMLESS_VOICE_WAKEUP
				/* Continuous Voice Sense Preset is 7001 
				*  ASRC OFF, for Audience VS
				*/
				dev_info(es705_priv.dev, "%s(): CVS preset = %d\n",
					__func__, es705_priv.cvs_preset);
				es705_cmd(&es705_priv, 0x906f1b59);
				es705_priv.cvs_preset = 0x1b59;

				dev_info(es705_priv.dev, "%s(): VS Keyword length = 0x%08x\n",
					__func__, vs_keyword_length_cmd);
...
#if defined(AUDIENCE_CVQ_DEBUG)
					/* memory check */
					es705_write_then_read(&es705_priv, &memorycheck_cmd,
								sizeof(memorycheck_cmd), &sync_rspn, 0);
					dev_info(es705_priv.dev, "%s(): memory check rspn = 0x%x\n", __func__, sync_rspn);
#endif
				}
			}
...
static void es705_event_status(struct es705_priv *es705)
{
	...
	u32 sync_ack;
	u32 sync_slim_cmd = (0x8070 << 16) | 0x0000;
	u32 calib_slim_cmd = (0x8071 << 16) | 0x0000;
	u32 slim_cmd_resp;
	u16 configure_uart = cpu_to_be32(ES_NO_EVENT);
	u32 preset_cmd = 0x90311bbc; /* PRESET cmd */
	u32 get_status = 0x8012 << 16 | 0x0000;
	u32 resp;
...
#if defined(AUDIENCE_CVQ_DEBUG)
	dev_info(es705->dev, "%s(): debug chip crash from VS to NS\n", __func__);
	if (es705->pdata->esxxx_clk_cb) {
		es705->pdata->esxxx_clk_cb(1);
		usleep_range(3000, 3100);
	...
	es705_uart_es705_wakeup(es705);
	dev_info(es705->dev, "%s(): toggle wake up pin\n", __func__);	
	msleep(40);
	es70x_msg_sync();
	usleep_range(5000, 5100);
	es70x_fw_building_string();

	es_set_power_level = cpu_to_le32(es_set_power_level);
	rc = es705_write_then_read(es705,
			&es_set_power_level,
			sizeof(es_set_power_level),
			&rspn, 0);
	if (rc) {
		dev_err(es705->dev, "%s(): es705 power level 6 failed\n", __func__);
		return;
	}
	else
		dev_info(es705->dev, "%s(): send set normal power state\n", __func__);	
	usleep_range(5000, 5100);
	es70x_msg_sync();
	usleep_range(5000, 5100);
	es70x_fw_building_string();
	...
	if (es705->es705_power_state == ES705_SET_POWER_STATE_VS_LOWPWR) {
		dev_info(es705->dev, "%s(): wakeup the chip form ES705_SET_POWER_STATE_VS_LOWPWR\n", __func__);
		rc = es705_wakeup(es705);
		if (rc) {
	...
	if ((event_response & 0x00FF) == KW_DETECTED) {
			...
			do {
				...
				rc = es705->cmd(es705, sync_slim_cmd, 0, &slim_cmd_resp);
				if (rc)
					pr_err("%s(): sending configure command on slim failed",
							__func__);

				rc = es705_uart_write(es705, &configure_uart,
							sizeof(configure_uart));
				if (rc < 0)
					pr_err("%s(): sending configure command on uart failed",
							__func__);

				usleep_range(5000, 5005);
				rc = es705_uart_cmd(es705, sync_cmd, 0, &sync_ack);
				if (rc < 0)
					pr_err("%s(): sending sync command failed - %d\n",
							__func__, rc);
				else
					pr_err("%s(): sync ack is = 0x%08x",
							__func__, ES_SYNC_ACK);

				rc = es705->cmd(es705, calib_slim_cmd, 0, &slim_cmd_resp);
				if (rc) {
					pr_err("%s(): sending caliberation command on slim failed",
							__func__);
				} else if (slim_cmd_resp == calib_slim_cmd) {
					pr_debug("%s(): caliberation was successful",
							__func__);
					break;
				} else {
					pr_debug("%s(): caliberation response 0x%08x",
							__func__, slim_cmd_resp);
				}

			} while (sync_retry--);

			pr_debug("%s(), number of tries made : %d",
				__func__, ES_SYNC_MAX_RETRY - sync_retry + 1);

			if (!sync_retry) {
				pr_err("%s(): UART internal clock is failed to configure, using fallback mechanism",
						__func__);
				...
				pr_info("%s(): calibrating the uart was successful", __func__);
			}

			pr_debug("%s(), closing the UART", __func__);
...
voiceq_isr_exit:
...
#if 0
			/* Send Preset #7100 */
			dev_info(es705->dev, "%s(): Send Preset(#7100)\n", __func__);
			es705_cmd(es705, 0x90311bbc);
#endif
		...
		{
			cpu_to_le32s(chk_vs_keyword);
			rc = es705_write_then_read(es705, &chk_vs_keyword,
					sizeof(chk_vs_keyword),
					&rspn, 0);
			if (rc) {
				dev_err(es705->dev,
						"%s(): check vs keyword failed\n",
						__func__);
				return;
			}
			le32_to_cpus(&rspn);
			es705_priv.detected_vs_keyword = rspn;
		}
